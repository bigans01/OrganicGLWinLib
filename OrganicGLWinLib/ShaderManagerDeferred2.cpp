...
void ShaderManagerDeferred2::initializeShader(int in_windowWidth, int in_windowHeight, int in_terrainBufferSize)
{
	...
	// load shaders
	OrganicGLWinUtils::loadShadersViaMode(&shaderProgramID, 3);		// load mode 5 shaders;	(3 will become 5, but only during testing, until OGLM functionality is revisited -- BE SURE to set the appropriate shader back when done!!)
