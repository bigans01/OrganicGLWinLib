...
class CutLinePool
{
	public:
		...
		CutLine fetchLineFromPoolViaIndex(int in_lineIndex)
		{
			return pool[in_lineIndex];
		}
		CutLine fetchLastLineInPool()
		{
			auto lastLine = pool.rbegin();
			return lastLine->second;
		}
		std::map<int, CutLine>::iterator getPoolIteratior(int in_lineIndex)
		{
		...
		void replaceLinesAtStartingIndexAndUpdate(int in_startingIndex, CutLine in_line)
		{
			std::map<int, CutLine> updatedPool;
	...
	private:
		...
		void incrementIterator(std::map<int, CutLine>::iterator* in_iterator)
		{
