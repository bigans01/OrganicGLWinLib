...
GLuint OrganicShaderLoader::LoadShaders(const char* vertex_file_path, const char* fragment_file_path)
{
	// Create the shaders
...
GLBasicProgramData OrganicShaderLoader::createBasicShaderProgram(const char* vertex_file_path, const char* fragment_file_path)
{
	// Create the shaders
	GLuint VertexShaderID = glCreateShader(GL_VERTEX_SHADER);
	GLuint FragmentShaderID = glCreateShader(GL_FRAGMENT_SHADER);
	...
	if (VertexShaderStream.is_open()) {
		...
		printf("Impossible to open %s. Are you in the right directory ? Don't forget to read the FAQ !\n", vertex_file_path);
		getchar();
		return GLBasicProgramData(0, 0, -1);
	}
	...
	return GLBasicProgramData(VertexShaderID, FragmentShaderID, ProgramID);
}
