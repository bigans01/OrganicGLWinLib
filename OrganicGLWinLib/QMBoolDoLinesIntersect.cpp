...
bool QMBoolDoLinesIntersect::solve(QuatRotationPoints* in_quatRotationPointsRef, PolyDebugLevel in_polyDebugLevel)
{
	...
	if (checkForExactPointMatch(in_quatRotationPointsRef) == false)
	{
				...
				)
		{
			...
			//std::cout << std::setprecision(8) << "Calc'd point x: " << calculatedPoint.x << " | line A, point B x: " << in_quatRotationPointsRef->getPointByIndex(1).x << std::endl;
	...
	else   // the point match was true; so set the returnValue to true.
	{
		returnValue = true;
	}
	return returnValue;
...
bool QMBoolDoLinesIntersect::checkForExactPointMatch(QuatRotationPoints* in_quatRotationPointsRef)
{
	bool wasPointMatchFound = false;
	// check if one of the following is met:
	// Line A, point A equals Line B, point A
	// Line A, point A equals Line B, point B
	// Line A, point B equals Line B, point A

	glm::vec3 lineAPointA = in_quatRotationPointsRef->getPointByIndex(0);
	glm::vec3 lineAPointB = in_quatRotationPointsRef->getPointByIndex(1);
	glm::vec3 lineBPointA = in_quatRotationPointsRef->getPointByIndex(2);
	glm::vec3 lineBPointB = in_quatRotationPointsRef->getPointByIndex(3);

	if
	(
			lineAPointA == lineBPointA
	)
	{
		wasPointMatchFound = true;
		glm::vec3* intersectedPointRef = in_quatRotationPointsRef->getPointRefByIndex(4);
		*intersectedPointRef = lineAPointA;
	}
	else if
	(
			lineAPointA == lineBPointB
	)
	{
		wasPointMatchFound = true;
		glm::vec3* intersectedPointRef = in_quatRotationPointsRef->getPointRefByIndex(4);
	...
	(
		lineAPointB == lineBPointA
	)
	{
		wasPointMatchFound = true;
		glm::vec3* intersectedPointRef = in_quatRotationPointsRef->getPointRefByIndex(4);
		*intersectedPointRef = lineAPointB;
	}
	else if
	(
		lineAPointB == lineBPointB
	)
	{
		wasPointMatchFound = true;
		glm::vec3* intersectedPointRef = in_quatRotationPointsRef->getPointRefByIndex(4);
