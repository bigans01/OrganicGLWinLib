...
{
	// find the centroid
	glm::vec3 pointA = in_sTriangle.triangleLines[0].fetchPointAGlmVec3Version();
	glm::vec3 pointB = in_sTriangle.triangleLines[1].fetchPointAGlmVec3Version();
	glm::vec3 pointC = in_sTriangle.triangleLines[2].fetchPointAGlmVec3Version();
	...
	for (int x = 0; x < 3; x++)
	{
		glm::vec3 centroidFacingNormal = QuatUtils::findOrientatedLineNormal(in_sTriangle.triangleLines[x].fetchPointAGlmVec3Version(),
																			in_sTriangle.triangleLines[x].fetchPointBGlmVec3Version(),
																			centroid);

		centroidFacingNormal *= -1;	// outward facing normal is just the centroid normal multiplied by -1.
		
		CuttingTriangleLine newLine(in_sTriangle.triangleLines[x].fetchPointAGlmVec3Version(),
									in_sTriangle.triangleLines[x].fetchPointBGlmVec3Version(),
